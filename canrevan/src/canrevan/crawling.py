            ...
            with open(filename, "w", encoding='utf-8') as fp:
                ...
                def callback_fn(data: Optional[T]):
                    ...
                    if data is not None:
                        ...
                        nonlocal written
                        written += 1
                        query = "INSERT INTO stage_news_one (title, content, news_date) VALUES (%s, %s, %s)"
                        cursor.execute(query, (data[0],  data[1], datetime.strptime(data[2].replace('"',""),"%Y.%m.%d")))
                        fp.write(str(data[2]) + "\x01")
                        fp.write(str(data[0]) + "\x01")
                        fp.write(str(data[1]) + "\n")
